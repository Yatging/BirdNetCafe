...
// ---------- 3. 鸟类列表渲染（新增功能）----------
function renderBirdSpeciesList() {
  const listContainer = document.getElementById('bird-species-list');
  ...
  tree.birds.forEach((bird, index) => {
    ...
    li.textContent = `${bird.species}  (${(bird.confidence * 100).toFixed(0)}%)`;
    li.dataset.birdIndex = index;

    li.addEventListener('mouseenter', () => {
      const birdNodes = document.querySelectorAll('.bird-node');
      if (birdNodes[index]) {
        birdNodes[index].classList.add('bird-highlight');
      }
      showPopup(bird);
    });
    ...
    li.addEventListener('mouseleave', () => {
      const birdNodes = document.querySelectorAll('.bird-node');
      if (birdNodes[index]) {
        birdNodes[index].classList.remove('bird-highlight');
      }
      closePopup();
    });
...
function initMainMap() { 
  ...
  map = L.map('map').setView([22.340380, 114.163472], 13);
...
function openCreateModal() {
  ...
  setTimeout(() => {
    if (!modalMap) {
      modalMap = L.map('modal-map-container').setView([22.340380, 114.163472], 11);
...
function closePopup() {
  ...
  // 同时移除所有鸟节点的高亮状态
  document.querySelectorAll('.bird-node').forEach(n => n.classList.remove('bird-highlight'));
}
